...
def check_config():
    ...
    try:
        ...
        report.write("  swan-provider config file is not exist! \n")
    ...
    try:
        ...
        report.write("  boost config file is not exist! \n")
...
def health_aria2():
    ...
    try:
        ...
        if r.status_code != 200:
            report.write(
                "  1. Check the aria2 rpc service fail! return data is " + r.content + ", Please check aria2_host、aria2_port、aria2_secret.")
        else:
            report.write("  1. Check the aria2 rpc service success. \n")
    except:
        report.write("  1. Check the aria2 rpc service fail! \n")
...
def health_swan():
    ...
    try:
        ...
        if r.status_code != 200:
            report.write(
                "  2. Check the connectivity with swan platform fail! return data is " + r.content + ", Please check api_key and access_token! \n")
        else:
            report.write("  2. Check the connectivity with swan platform success. \n")
    except:
        report.write("  2. Check the connectivity with swan platform fail! \n")
...
def health_lotus():
    ...
    try:
        ...
        if r_lotus.status_code != 200:
            report.write(
                "  3. Check the connectivity with lotus fail! return data is " + r_lotus.text + ", Please check client_api_url and client_api_token! \n")
        else:
            report.write("  3. Check the connectivity with lotus success. \n")
    except:
        report.write("  3. Check the connectivity with lotus fail! \n")
    try:
        data = {
            "jsonrpc": "2.0",
        ...
        if r_miner.status_code != 200:
            report.write(
                "  4. Check the connectivity with lotus-miner fail! return data is " + r_miner.text + ", Please check market_api_url and market_access_token! \n")
        else:
            report.write("  4. Check the connectivity with lotus-miner success. \n")
    except:
        report.write("  4. Check the connectivity with lotus-miner fail! \n")
    ...
    try:
        ...
        if market_version == '1.2':
            ...
            if r_boost.status_code != 200:
                report.write(
                    "  5. Check the connectivity with boost fail! return data is " + r_boost.text + ", Please check boost service process. \n")
            else:
                report.write("  5. Check the connectivity with boost success. \n")
    except:
        report.write("  5. Check the connectivity with boost fail! \n")
        ...
        for key, value in parsed_toml.items():
            ...
            if key == "aria2":
                ...
                if len(value['aria2_download_dir']) == 0:
                    report.write("  5.  aria2.aria2_download_dir is null! \n")
                elif not os.path.exists(value['aria2_download_dir']):
                    report.write("  5.  aria2.aria2_download_dir not exist, need to be created manually! \n")
                else:
                    report.write("  5.  aria2.aria2_download_dir is ok. \n")
                if not isinstance(aria2_candidate_dirs, (list, str)):
                    report.write(
                        '  6.  aria2.aria2_candidate_dirs is null, allow two format: ["/tmp", "/data"] or "/tmp, /data" ! \n')
                    ...
                    for r_dir in results:
                        if not os.path.exists(r_dir.strip()):
                            report.write(
                                "  6.  aria2.aria2_candidate_dirs, directory: " + r_dir.strip() + " not exist! \n")
            ...
            if key == "bid":
                ...
                if bid_mode not in [0, 1]:
                    report.write("  16. bid.bid_mode can only be 0 or 1! \n")
                else:
                    if bid_mode == 0:
                        report.write(
                ...
                if not isinstance(expected_sealing_time, numbers.Number):
                    report.write("  17. bid.expected_sealing_time is null! \n")
                elif 1920 > expected_sealing_time > 2880:
                    report.write("  17. bid.expected_sealing_time range is [1920~2880]! \n")
                else:
                    report.write("  17. bid.expected_sealing_time is ok. \n")
                if not isinstance(value['start_epoch'], numbers.Number):
                    report.write("  18. bid.start_epoch is null! \n")
                else:
                    report.write("  18. bid.start_epoch is ok. \n")
                if not isinstance(auto_bid_deal_per_day, numbers.Number):
                    report.write("  19. bid.auto_bid_deal_per_day is null! \n")
                elif auto_bid_deal_per_day < 500:
                    report.write("  19. bid.auto_bid_deal_per_day value must be >= 500! \n")
                else:
            ...
            if market_version == '1.2':
                ...
                if key == "market":
                    ...
                    if len(collateral_wallet) == 0:
                        report.write("  20. market.collateral_wallet is null! \n")
                    ...
                    if len(publish_wallet) == 0:
                        report.write("  21. market.publish_wallet is null! \n")
...
def check_query():
    ...
    if r.status_code != 200:
        report.write("  Check miner query-ask failed! return data is " + r.content + " \n")
    else:
        parsed_toml = toml.load(os.path.join(swan_path, "provider/config.toml"))
        market_version = ''
        for key, value in parsed_toml.get('main').items():
        ...
        if result['status'] == "fail":
            if market_version == '1.1':
                report.write("  Please use the command 'lotus-miner net listen' to query multi-address!")
            if market_version == '1.2':
                report.write(
                    "  Please use the command 'boostd --boost-repo=$SWAN_PATH/provider/boost net listen' to query multi-address! \n")
        elif result['status'] == "success":
            if result['data']['price_per_GiB'] != "0" or result['data']['verified_price_per_GiB'] != "0":
                if market_version == '1.1':
                    report.write("Please use the command 'lotus-miner storage-deals set-ask --price 0 "
                                 "--verified-price 0 --min-piece-size 56KiB --max-piece-size 64GB' to set the price! \n")
                if market_version == '1.2':
                    report.write("  Please use the command 'export SWAN_PATH=$SWAN_PATH && swan-provider set-ask "
                                 "--price=0 --verified-price=0 --min-piece-size=256 --max-piece-size=34359738368'"
                                 " to set the price! \n")
            else:
