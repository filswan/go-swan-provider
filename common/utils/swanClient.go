...
func (self *SwanClient) UpdateOfflineDealStatus(dealId int, status, note string, fileInfo ...string) (string) {
	...
	if len(fileInfo) > 0 {
		form.Add("file_path", fileInfo[0])
	}

	if len(fileInfo) > 1 {
		form.Add("file_size", fileInfo[1])
	}
